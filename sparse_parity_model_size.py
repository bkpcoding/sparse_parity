...
def cfg():
    ...
    # D = -1
    base_width = 100  # renamed from width
    depths = [2, 4, 6, 8, 10]  # list of depths to compare
    activation = 'ReLU'
...
@ex.automain
def run(n_tasks, n, k, alpha, offset, D, base_width, depths,
    ...
    # Generate tasks with one-bit differences instead of random tasks
    Ss = get_one_bit_different_tasks(n, k, n_tasks)
    print(f"Tasks: {Ss}")
    
    # Initialize dictionary to store results for each depth
    results = {}
    
    # Train models of different depths
    for depth in depths:
        print(f"\nTraining model with depth {depth}")
        mlp = create_model(n_tasks, n, base_width, depth, activation_fn, device, dtype)
        steps_to_converge = train_model(
            mlp, Ss, n_tasks, n, k, alpha, offset, D,
            batch_size, lr, weight_decay, test_points, test_points_per_task,
            steps, device, dtype, log_freq, stop_early, verbose
        )
        
        print(f"Depth {depth} converged in {steps_to_converge} steps")
        results[f'depth_{depth}'] = steps_to_converge
        ex.info[f'convergence_steps_depth_{depth}'] = steps_to_converge
    
    # Print comparison summary
    print("\nModel Size Comparison:")
    for depth in depths:
        print(f"Depth {depth}: {results[f'depth_{depth}']} steps")
