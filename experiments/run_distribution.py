...
@hydra.main(config_path="../configs", config_name="experiment/distribution_experiment")
def run_distribution_experiment(cfg: DictConfig):
    """Run experiment comparing uniform vs Zipf distribution for task sampling."""
    ...
    if cfg.experiment.use_wandb:
        wandb.init(
            ...
            config=dict(cfg),
            name=f"distribution_comparison_{cfg.experiment.comparison_mode}"
        )
    ...
    # Generate task subsets (same for both distributions)
    task_subsets = get_random_tasks(cfg.n, cfg.k, cfg.n_tasks)
    print(f"Task subsets: {task_subsets}")
    ...
    for ensemble_idx in range(cfg.n_ensembles):
        ...
        # Run uniform distribution comparison
        if cfg.experiment.comparison_mode in ['uniform', 'both']:
            cfg.experiment.distribution_type = "uniform"
        ...
        # Run Zipf distribution comparison
        if cfg.experiment.comparison_mode in ['zipf', 'both']:
            cfg.experiment.distribution_type = "zipf"
            ...
            steps, loss_data = trainer.train()
            ensemble_results['zipf'].append(steps)

            if is_last_ensemble and loss_data and cfg.plot_losses:
                log_steps, losses_subtasks = loss_data
                create_loss_animation(losses_subtasks, log_steps, 'zipf')
        ...
        for method in ensemble_results:
            if len(ensemble_results[method]) > 0:
                ...
                print(f"{method.upper()} distribution:")
    ...
    # Log comparison metrics if running both distributions
    if cfg.experiment.comparison_mode == 'both':
        uniform_mean = results['uniform_mean']
        zipf_mean = results['zipf_mean']
        mean_diff = abs(uniform_mean - zipf_mean)
        print(f"\nMean difference (Uniform vs Zipf): {mean_diff:.2f} steps")
